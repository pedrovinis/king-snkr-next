...
export default class Snkr {
    ...
    setSnkrEdition(edition) {
        this.snkrData['edition'] = edition
    }

    getSnkrEdition() {
        return this.snkrData['edition']
    }

    setSnkrImage(image) {
        this.snkrData['image'] = image
    }

    getSnkrImage() {
        return this.snkrData['image']
    }
    ...
    saveConfigs() {
        fs.writeFileSync(`bin/snkrs/${this.snkrData.name}-${this.snkrData.edition}-${this.snkrData.id}.json`, JSON.stringify(this.snkrData), 'utf8', ()=> {
        }) 
    }
}